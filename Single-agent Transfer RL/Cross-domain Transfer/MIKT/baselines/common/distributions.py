...
class CategoricalPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            pdparam = _matching_fc(latent_vector, 'pi_out', self.ncat, init_scale=init_scale, init_bias=init_bias)
...
class MultiCategoricalPdType(PdType):
    ...
    def pdfromlatent(self, latent, init_scale=1.0, init_bias=0.0):
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            pdparam = _matching_fc(latent, 'pi_out', self.ncats.sum(), init_scale=init_scale, init_bias=init_bias)
        return self.pdfromflat(pdparam), pdparam
...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            mean = _matching_fc(latent_vector, 'pi_out', self.size, init_scale=init_scale, init_bias=init_bias)
...
class BernoulliPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            pdparam = _matching_fc(latent_vector, 'pi_out', self.size, init_scale=init_scale, init_bias=init_bias)
        return self.pdfromflat(pdparam), pdparam
...
def make_pdtype(ac_space, scope):
    from gym import spaces
    if isinstance(ac_space, spaces.Box):
        assert len(ac_space.shape) == 1
        return DiagGaussianPdType(ac_space.shape[0], scope)
    elif isinstance(ac_space, spaces.Discrete):
        return CategoricalPdType(ac_space.n, scope)
    elif isinstance(ac_space, spaces.MultiDiscrete):
        return MultiCategoricalPdType(ac_space.nvec, scope)
    elif isinstance(ac_space, spaces.MultiBinary):
        return BernoulliPdType(ac_space.n, scope)
    else:
        raise NotImplementedError
