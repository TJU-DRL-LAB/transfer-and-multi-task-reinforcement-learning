...
def load_results(root_dir_or_dirs, enable_progress=True, enable_monitor=True, verbose=False):
    ...
    从progress.csv files中下载数据
    enable_monitor: bool - if True, will attempt to load data from monitor.csv files (data saved by Monitor environment wrapper). Default: True
    从monitor.csv files中下载数据？这个文件是什么？
...
def progress_xy_rew_fn(r):
    x = r.progress.total_timesteps
    y = smooth(r.progress.eprewmean, radius=20)
    # y = r.progress.eprewmean
    return x, y


def progress_xy_ps_fn(r):
    x = r.progress.total_timesteps
    y = smooth(r.progress.student_ps, radius=1)
    return x, y
...
def plot_transfer_results(
    ...
    for results, key in zip(results_list, keys):
        for result in results:
            sk2r[key].append(result)
    '''
    for result in student_results:
        sk2r[student_key].append(result)
    for result in teacher_results:
        sk2r[teacher_key].append(result)
    '''
    assert len(sk2r) > 0
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        for result in sresults:
            ...
            if average_group:
                ...
                l, = ax.plot(x, y, color=COLORS[groups.index(group) %
                                                len(COLORS)])
                g2l[group] = l
        if average_group:
            for group in sorted(groups):
                ...
                if resample:
                    ...
                    ys = []
                    for (x, y) in xys:
                        ys.append(symmetric_ema(x, y, low, high, resample,
                ...
                l, = axarr[0][0].plot(usex, ymean, color=color)
                g2l[group] = l
                if shaded_err:
...
def plot_results(  # 结果图
    ...
    xy_fn: function Result -> x,y           - function that converts results objects into tuple of x and y values.将结果对象转换为x和y值的元组
