...
def train(args, extra_args):
    ...
    learn = get_learn_function(args.alg)  # PPO
    alg_kwargs = get_learn_function_defaults(args.alg, env_type)  # default parameters
    ...
    if args.source_env is not None:
        ...
        source_env = None
    if args.source_env1 is not None:
        source_env_type1, source_env_id1 = get_env_type(args.source_env1)
        source_env1 = build_source_env1(args)
    else:
        source_env1 = None
    ...
    teacher_network, teacher_paths, teacher_paths1 = args.teacher_network, args.teacher_paths, args.teacher_paths1
    save_path, log_path = args.save_path, args.log_path
    save_filenumber = args.save_filenumber
    load_path = args.load_path
    transfer, transfer_env = args.transfer, args.transfer_env
    mapping = args.mapping
    ps_coef, vf_ps_coef, ps_coef1, vf_ps_coef1, mutual_info_coef, mutual_info_coef1, kl_coef = None, None, None, None, None, None, None
    if teacher_paths is not None and teacher_paths1 is not None:
        ...
        ps_coef1, vf_ps_coef1 = random.choice(ps_coefs), random.choice(vf_ps_coefs)
        mutual_info_coef = random.choice(mutual_info_coefs)
        mutual_info_coef1 = mutual_info_coef
        kl_coef = random.choice(kl_coefs)
    model = learn(
        env=env,
        source_env=source_env,
        source_env1=source_env1,
        seed=seed,
        pi_scope=pi_scope,
        vf_scope=vf_scope,
        teacher_paths=teacher_paths,
        teacher_paths1=teacher_paths1,
        teacher_network=teacher_network,
        total_timesteps=total_timesteps,
        save_path=save_path,
        save_filenumber=save_filenumber,
        load_path=load_path,
        log_path=log_path,
        transfer=transfer,
...
#  build environments
def build_env(args):
    ncpu = multiprocessing.cpu_count()  # CPU keras
...
def build_source_env1(args):
    ...
    env_type, env_id = get_env_type(args.source_env1)
