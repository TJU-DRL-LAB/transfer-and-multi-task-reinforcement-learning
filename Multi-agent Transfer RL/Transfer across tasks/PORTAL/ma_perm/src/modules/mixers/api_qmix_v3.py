...
class HYPER_W1(nn.Module):
    def __init__(self, args):
        ...
        self.layer_1 = APIHyperInputLayer(args, args.hypernet_embed)
        self.layer_2 = APIHyperOutputLayer(args, self.embed_dim)

    def forward(self, qvals, features):
        """
        ...
        :return:
        """
        hyper_w1_hidden = self.layer_1.forward(features)  # [bs * t, output_dim]
        activated_hidden = F.relu(hyper_w1_hidden, inplace=True)
        output = self.layer_2(activated_hidden, qvals.view(-1, 1))
        return output
...
class APIQMixer(nn.Module):
    ...
    def forward(self, qvals, states):
        ...
        # First layer
        w1 = self.hyper_w1(qvals, features).view(-1, self.n_agents, self.embed_dim)  # b * t, n_agents, emb
