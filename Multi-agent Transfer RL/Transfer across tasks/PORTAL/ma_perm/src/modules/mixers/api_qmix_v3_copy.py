...
class APIHyperInputLayer(nn.Module):
    def __init__(self, args, output_dim, return_features=False):
    ...
    def forward(self, features):
        ...
        if self.return_features:
            return F.relu(output, inplace=True), features
        return output
...
class APIHyperOutputLayer(nn.Module):
    ...
    def forward(self, hidden_and_features):
        hidden, features = hidden_and_features
        # [bs * t * n_agents, fea_dim]
        ally_features, enemy_features = features

        # [bs * t, n_agents, hypernet_embed, output_dim] -> [bs * t, hypernet_embed, n_agents, output_dim]
        w_ally = self.hyper_w_ally(ally_features).view(
        ...
        q_weight = th.matmul(hidden.unsqueeze(1), w_ally).view(
            -1, self.n_agents, self.output_dim
...
class APIQMixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        # hyper w1 b1
        self.hyper_w1 = nn.Sequential(APIHyperInputLayer(args, args.hypernet_embed, return_features=True),
                                      APIHyperOutputLayer(args, self.embed_dim))
