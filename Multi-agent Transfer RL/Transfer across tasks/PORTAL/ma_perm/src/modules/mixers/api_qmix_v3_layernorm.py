...
class APIHyperInputLayer(nn.Module):
    def __init__(self, args, output_dim):
        ...
        self.hyper_w_enemy_1 = nn.Linear(args.state_enemy_feats_size, args.api_hyper_dim)
        self.hyper_w_enemy_2 = nn.LayerNorm([self.n_enemies, args.api_hyper_dim])
        self.hyper_w_enemy_3 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, args.state_enemy_feats_size * output_dim)
        )  # output shape: (enemy_feats_dim * output_dim)

        self.hyper_w_ally_1 = nn.Linear(args.state_ally_feats_size, args.api_hyper_dim)
        self.hyper_w_ally_2 = nn.LayerNorm([self.n_agents, args.api_hyper_dim])
        self.hyper_w_ally_3 = nn.Sequential(
            nn.ReLU(inplace=True),
    ...
    def forward(self, features):
        ...
        hyper_hidden_ally = self.hyper_w_ally_1(ally_features).view(
            -1, self.n_agents, self.args.api_hyper_dim
        )  # [b * t, n_agents, api_hyper_dim]
        hyper_hidden_ally = self.hyper_w_ally_2(hyper_hidden_ally).view(-1, self.args.api_hyper_dim)
        w_ally = self.hyper_w_ally_3(hyper_hidden_ally).view(-1, self.args.state_ally_feats_size, self.output_dim)
        ...
        hyper_hidden_enemy = self.hyper_w_enemy_1(enemy_features).view(
            -1, self.n_enemies, self.args.api_hyper_dim
        )  # [b * t, n_enemies, api_hyper_dim]
        hyper_hidden_enemy = self.hyper_w_enemy_2(hyper_hidden_enemy).view(-1, self.args.api_hyper_dim)
        w_enemy = self.hyper_w_enemy_3(hyper_hidden_enemy).view(-1, self.args.state_enemy_feats_size, self.output_dim)
