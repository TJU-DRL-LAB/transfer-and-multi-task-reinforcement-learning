...
class APIQMixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        # hyper w1 and w2, share the input embedding layer
        self.hyper_w1_and_w2 = nn.Sequential(APIHyperInputLayer(args, args.hypernet_embed),
                                             nn.ReLU(inplace=True),
                                             nn.Linear(args.hypernet_embed, (self.n_agents + 1) * self.embed_dim))
    ...
    def forward(self, qvals, states):
        ...
        w1_and_w2 = self.hyper_w1_and_w2(features).view(-1, self.n_agents + 1, self.embed_dim)

        # First layer  [b * t, (n_agents + 1) * embed_dim]
        w1 = w1_and_w2[:, :-1]  # [b * t, n_agents, emb]
        b1 = self.hyper_b1(features).view(-1, 1, self.embed_dim)

        # Second layer
        w2 = w1_and_w2[:, -1].unsqueeze(dim=-1)  # [b * t, emb, 1]
