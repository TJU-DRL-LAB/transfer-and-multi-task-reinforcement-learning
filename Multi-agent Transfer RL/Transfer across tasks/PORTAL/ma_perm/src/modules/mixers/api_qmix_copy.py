...
class APIHyperLayer1(nn.Module):
    def __init__(self, args):
        ...
        self.hyper_w_and_b_ally = nn.Sequential(
            ...
            nn.Linear(args.api_hyper_dim, 2 * self.embed_dim)
        ...
        #     nn.ReLU(inplace=True),
        #     nn.Linear(args.api_hyper_dim, (args.state_enemy_feats_size + 1) * self.embed_dim)
...
class APIHyperLayer2(nn.Module):
    def __init__(self, args):
        ...
        self.hyper_w_and_b_ally = nn.Sequential(
            nn.Linear(args.state_ally_feats_size, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, self.embed_dim * 1 + 1)
...
class APIHyperWeight(nn.Module):
    def __init__(self, args):
        super(APIHyperWeight, self).__init__()
        self.args = args
        self.n_agents = args.n_agents
        self.n_enemies = args.n_enemies
