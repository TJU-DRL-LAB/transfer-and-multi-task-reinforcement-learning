...
class HYPER_W1(nn.Module):
    def __init__(self, args, output_dim):
        ...
        self.layer_1 = nn.Linear(args.state_ally_feats_size, args.api_hyper_dim)
        self.layer_2 = nn.InstanceNorm1d(num_features=args.api_hyper_dim)
        self.layer_3 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, output_dim)
        )  # output shape: (ally_feats_dim * output_dim)
        # nn.GroupNorm()
    ...
    def forward(self, features):
        ...
        """
        ally_features, enemy_features = features
        hidden = self.layer_1(ally_features).view(
            -1, self.n_agents, self.args.api_hyper_dim
        ).transpose(-1, -2)  # [b * t, n_agents, api_hyper_dim] -> [b * t, api_hyper_dim, n_agents]
        hidden = self.layer_2(hidden).transpose(-1, -2).reshape(-1, self.args.api_hyper_dim)
        output = self.layer_3(hidden)  # [b * t * n_agents, output_dim]
        return output
