...
class APIQMixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        assert self.embed_dim == args.hypernet_embed
        input_layer = APIHyperInputLayer(args, args.hypernet_embed)

        # hyper w1 b1
        self.hyper_w1 = nn.Sequential(input_layer,
                                      nn.ReLU(inplace=True),
                                      nn.Linear(args.hypernet_embed, self.embed_dim * self.n_agents))
        self.hyper_b1 = nn.Sequential(input_layer)

        # hyper w2 b2
        self.hyper_w2 = nn.Sequential(input_layer,
                                      nn.ReLU(inplace=True),
                                      nn.Linear(args.hypernet_embed, self.embed_dim))
        self.hyper_b2 = nn.Sequential(input_layer,
                                      nn.ReLU(inplace=True),
