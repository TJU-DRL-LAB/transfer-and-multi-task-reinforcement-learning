...
class HYPER_W1(nn.Module):
    def __init__(self, args, output_dim):
        super(HYPER_W1, self).__init__()
        self.args = args
        self.n_agents = args.n_agents
        self.n_enemies = args.n_enemies
        self.n_actions = args.n_actions
        ...
        # TODO: add attention
        self.w_query_ally = nn.Linear(args.state_ally_feats_size, args.api_hyper_dim)

        self.w_key_ally = nn.Linear(args.state_ally_feats_size, args.api_hyper_dim)
        self.w_key_enemy = nn.Linear(args.state_enemy_feats_size, args.api_hyper_dim)

        self.softmax = nn.Softmax(dim=-1)

        self.w_value_ally = nn.Sequential(
            nn.Linear(args.state_ally_feats_size, args.api_hyper_dim),
            nn.LeakyReLU()
        )
        self.w_value_enemy = nn.Sequential(
            nn.Linear(args.state_enemy_feats_size, args.api_hyper_dim),
            nn.LeakyReLU()
        )

        self.hyper_w = nn.Linear(args.api_hyper_dim, output_dim)  # output shape: (ally_feats_dim * output_dim)

    def forward(self, features):
        """
        :param features: [b * t * n_agents, fea_dim]
        :return:
        """
        ally_features, enemy_features = features

        ally_query = self.w_query_ally(ally_features).view(-1, self.n_agents, self.args.api_hyper_dim)

        ally_key = self.w_key_ally(ally_features).view(-1, self.n_agents, self.args.api_hyper_dim).transpose(-2, -1)
        enemy_key = self.w_key_enemy(enemy_features).view(-1, self.n_enemies, self.args.api_hyper_dim).transpose(-2, -1)

        ally_value = self.w_value_ally(ally_features).view(-1, self.n_agents, self.args.api_hyper_dim)
        enemy_value = self.w_value_enemy(enemy_features).view(-1, self.n_enemies, self.args.api_hyper_dim)
        values = th.cat([ally_value, enemy_value], dim=1)  # [b*t, n_agents+n_enemies, hidden]

        # [b*t, n_agents, hidden] * [b*t, hidden, n_agents+n_enemies] -> [b*t, n_agents, n_agents+n_enemies]
        ally_atten_logits = th.bmm(ally_query, th.cat([ally_key, enemy_key], dim=-1)) / (self.args.api_hyper_dim ** 0.5)

        atten_weights = self.softmax(ally_atten_logits)  # [b*t, n_agents, n_agents+n_enemies]

        weighted_values = th.bmm(atten_weights, values)  # [b*t, n_agents, hidden]

        output = self.hyper_w(weighted_values.view(-1, self.args.api_hyper_dim))  # [b * t * n_agents, output_dim]
        return output
...
class APIQMixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        # hyper w1 b1
        self.hyper_w1 = HYPER_W1(args, self.embed_dim)
