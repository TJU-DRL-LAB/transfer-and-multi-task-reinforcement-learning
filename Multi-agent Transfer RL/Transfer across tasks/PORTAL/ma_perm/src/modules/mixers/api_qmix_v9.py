...
class HYPER_W1(nn.Module):
    ...
    def forward(self, ally_features):
        """

        :param qvals: [b * t, 1, n_agents]
        :param features:
        :return:
        """
        output = self.hyper_w_ally(ally_features)   # [b * t * n_agents, output_dim]
        return output
...
class APIQMixer(nn.Module):
    ...
    def forward(self, qvals, states):
        ...
        # First layer
        w1 = self.hyper_w1(ally_features).view(-1, self.n_agents, self.embed_dim)  # b * t, n_agents, emb
