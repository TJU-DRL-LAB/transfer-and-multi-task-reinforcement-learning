...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.final_feats_net = nn.Sequential(
            nn.Linear(self.rnn_hidden_dim * 3, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, self.rnn_hidden_dim),
        )
    ...
    def forward(self, inputs, hidden_state):
        ...
        # (1) Own feature
        embedding_own = self.fc1_own(own_feats_t)  # [bs * n_agents, rnn_hidden_dim]
        query_own = embedding_own.unsqueeze(1).unsqueeze(1).expand(bs * self.n_agents, self.n_heads, 1, self.rnn_hidden_dim)
        ...
        # Q * K^T
        key_enemies = embedding_enemies.transpose(-1, -2)
        QK_enemy = th.matmul(query_own, key_enemies) # (bs*n_agent, n_heads, 1, n_enemies)
        embedding_enemies = th.matmul(self.attention_softmax(QK_enemy / np.sqrt(self.rnn_hidden_dim)), embedding_enemies)
        ...
        # Q * K^T
        key_allies = embedding_allies.transpose(-1, -2)
        QK_ally = th.matmul(query_own, key_allies) # (bs*n_agent, n_heads, 1, n_allies)
        embedding_allies = th.matmul(self.attention_softmax(QK_ally / np.sqrt(self.rnn_hidden_dim)), embedding_allies)
        embedding = self.final_feats_net(torch.cat([query_own, embedding_enemies, embedding_allies], dim=-1))
        # Final embedding
        embedding = self.unify_input_heads(embedding.squeeze(2))  # [bs * n_agents, head, rnn_hidden_dim]
