...
class DenseNetBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        hidden = F.relu(self.hidden_layer(x), inplace=True)  # input_dim
        output = th.cat([hidden, identity], dim=-1)  # input_dim * 2
        return output
...
class Hypernet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, depth):
        ...
        residual_blocks = []
        previous_output_dim = input_dim
        for i in range(depth):
            residual_blocks.append(DenseNetBlock(input_dim=previous_output_dim, output_dim=hidden_dim))
            previous_output_dim = previous_output_dim + hidden_dim
        self.residual_blocks = nn.Sequential(*residual_blocks)
        self.output_layer = nn.Linear(previous_output_dim, output_dim)

    def forward(self, x):
        # %%%%%%%%%%%%%%%%%%%%% Residual Blocks %%%%%%%%%%%%%%%%%%%%
        hidden = self.residual_blocks(x)
