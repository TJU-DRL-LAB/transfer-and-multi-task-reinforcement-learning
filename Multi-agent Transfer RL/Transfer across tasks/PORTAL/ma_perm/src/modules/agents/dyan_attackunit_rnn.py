...
class DyanRNNAgent(nn.Module):
    """
        DYAN with modified action network.
        Decode actions according to unit information.
    """
    def __init__(self, input_shape, args):
        super(DyanRNNAgent, self).__init__()
        self.args = args
        ...
        self.fc2_attack_actions = nn.Linear(args.rnn_hidden_dim + self.enemy_feats_dim, 1)  # output shape: rnn_hidden_dim * 1 + 1
        if self.args.map_type == "MMM":
            assert self.n_enemies >= self.n_agents
            self.fc2_rescue_actions = nn.Linear(args.rnn_hidden_dim + self.ally_feats_dim, 1)  # output shape: rnn_hidden_dim * 1 + 1
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        q_normal = self.fc2_normal_actions(h).view(bs, self.n_agents, -1)  # [bs, n_agents, 6]

        # Q-values of attack actions
        h_expands_enemies = h.unsqueeze(dim=1).expand(bs*self.n_agents, self.n_enemies, -1)
        h_cat_enemy_feats = th.cat((h_expands_enemies, enemy_feats_t.reshape(bs*self.n_agents, self.n_enemies, -1)), dim=-1)
        q_attack = self.fc2_attack_actions(h_cat_enemy_feats).view(bs, self.n_agents, -1)  # [bs, n_agents, n_enemies]
        if self.args.map_type == "MMM":
            h_expands_allies = h.unsqueeze(dim=1).expand(bs*self.n_agents, self.n_allies, -1)
            h_cat_ally_feats = th.cat((h_expands_allies, ally_feats_t.reshape(bs*self.n_agents, self.n_allies, -1)), dim=-1)
            q_rescue = self.fc2_rescue_actions(h_cat_ally_feats).view(bs, self.n_agents, -1) # [bs, n_agents, n_allies]
            # only support when the last agents are medivacs
