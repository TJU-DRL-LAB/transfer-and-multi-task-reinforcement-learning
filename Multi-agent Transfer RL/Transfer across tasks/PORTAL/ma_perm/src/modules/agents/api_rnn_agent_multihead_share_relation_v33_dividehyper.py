...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.relation_enemies = nn.ModuleList()
        ...
        for i in range(len(self.enemy_divide)):
            ...
            self.relation_enemies.append(nn.Sequential(
        ...
        self.relation_allies = nn.ModuleList()
        ...
        if self.args.map_type == "MMM":
            ...
            for i in range(1):
                ...
                self.relation_allies.append(nn.Sequential(
    ...
    def forward_for_feature(self, inputs, hidden_state):
        ...
        for i, enemy_feats_t in enumerate(enemy_feats_list):
            ...
            own_feats_t_expand_for_enemy = own_feats_t.unsqueeze(dim=1).expand(bs*self.n_agents, enemy_divide[i][0], self.own_feats_dim)
            enemy_feats_t_expand = enemy_feats_t.view(bs*self.n_agents, enemy_divide[i][0], self.enemy_feats_dim)
            relation_weight_enemy = self.relation_enemies[i](torch.cat([own_feats_t_expand_for_enemy, enemy_feats_t_expand], dim=2)).view(
                bs * self.n_agents, enemy_divide[i][0], self.n_heads, 1
            )
            embedding_enemies_i = embedding_enemies_i * relation_weight_enemy
        ...
        for i, ally_feats_t in enumerate(ally_feats_list):
            ...
            # [bs * n_agents * n_allies, 1, ally_fea_dim] * [bs * n_agents * n_allies, ally_fea_dim, rnn_hidden_dim] = [bs * n_agents * n_allies, 1, rnn_hidden_dim]
            own_feats_t_expand_for_ally = own_feats_t.unsqueeze(dim=1).expand(bs*self.n_agents, ally_divide[i][0], self.own_feats_dim)
            ally_feats_t_expand = ally_feats_t.view(bs*self.n_agents, ally_divide[i][0],self.ally_feats_dim)
            relation_weight_ally = self.relation_allies[i](torch.cat([own_feats_t_expand_for_ally, ally_feats_t_expand], dim=2)).view(
                bs * self.n_agents, ally_divide[i][0], self.n_heads, 1
            )
            embedding_allies_i = embedding_allies_i * relation_weight_ally
    ...
    def forward(self, inputs, hidden_state):
        ...
        q_attacks_type_list = []
        for num_of_type, hyper_enemy_out in zip(enemy_divide, hyper_enemies_out_list):
        ...
        if self.args.map_type == "MMM":
            hyper_ally_out = hyper_allies_out_list[0]
