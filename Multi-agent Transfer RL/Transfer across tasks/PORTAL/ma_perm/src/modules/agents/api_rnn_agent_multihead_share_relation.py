...
# share 是指前面的weight和后面动作的weight share同一个hypernet
# relation指在进行sum操作前，对各个实体的obs乘以一个relation weight
# 现在的其实是relation_v2
...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        # relation hypernet dim = enemy_feats_dim * 1
        # TODO: relation net是否需要隐藏层
        enemy_hyper_dim = ((self.enemy_feats_dim + 1) * self.rnn_hidden_dim + 1 + self.enemy_feats_dim) * self.n_heads
        ...
        if self.args.map_type == "MMM":
            ...
            # relation hypernet dim = enemy_feats_dim * 1
            # TODO: relation net是否需要隐藏层
            ally_hyper_dim = (self.ally_feats_dim * self.rnn_hidden_dim + self.ally_feats_dim) * self.n_heads
            self.hyper_ally = nn.Sequential(
                nn.Linear(self.ally_feats_dim, args.api_hyper_dim),
                nn.ReLU(inplace=True),
        ...
        self.relation_activation = torch.nn.ReLU()
    ...
    def forward(self, inputs, hidden_state):
        ...
        # (3) Enemy feature  (enemy_feats_dim * rnn_hidden_dim + rnn_hidden_dim + 1)
        hyper_enemy_out = self.hyper_enemy(enemy_feats_t) # (bs*n_agent*n_enemy, ((enemy_feats_dim * rnn_hidden_dim + rnn_hidden_dim + 1)*n_head)
        fc1_w_enemy = hyper_enemy_out[:, :-(self.rnn_hidden_dim + 1 + self.enemy_feats_dim) * self.n_heads].reshape(
            -1, self.enemy_feats_dim, self.rnn_hidden_dim * self.n_heads
        )  # [bs * n_agents * n_enemies, enemy_fea_dim, rnn_hidden_dim]
        # relation weight
        relation_w_enemy = hyper_enemy_out[:, -(self.rnn_hidden_dim + 1 + self.enemy_feats_dim) * self.n_heads: -(self.rnn_hidden_dim + 1) * self.n_heads].reshape(
            -1, self.enemy_feats_dim, 1 * self.n_heads
        )
        relation_enemy = th.matmul(enemy_feats_t.unsqueeze(1), relation_w_enemy).view(
