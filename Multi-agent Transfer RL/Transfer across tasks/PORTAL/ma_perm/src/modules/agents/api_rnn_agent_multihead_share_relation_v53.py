...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        # Q for own_feats
        self.WQ = nn.Linear(self.own_feats_dim, self.rnn_hidden_dim)
        # K for entity_feats
        self.WK_enemy = nn.Linear(self.enemy_feats_dim, self.rnn_hidden_dim)
        self.WK_ally = nn.Linear(self.ally_feats_dim, self.rnn_hidden_dim)
        # V for entity_feats
        self.WV_enemy = nn.Linear(self.enemy_feats_dim, self.rnn_hidden_dim)
        self.WV_ally = nn.Linear(self.ally_feats_dim, self.rnn_hidden_dim)
    ...
    def forward_for_feature(self, inputs, near_n):
        ...
        embedding_enemies = embedding_enemies.mean(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]
    ...
    def forward(self, inputs, hidden_state):
        ...
        # (1) Own feature
        embedding_own = self.fc1_own(own_feats_t)  # [bs * n_agents, rnn_hidden_dim]
        query_own = self.WQ(own_feats_t).unsqueeze(dim=1)
        # (2) ID embeddings
        if self.args.obs_agent_id:
            agent_indices = embedding_indices[0]
            # [bs * n_agents, rnn_hidden_dim]
        ...
        # [bs * n_agents * n_enemies, 1, enemy_fea_dim] * [bs * n_agents * n_enemies, enemy_fea_dim, rnn_hidden_dim] = [bs * n_agents * n_enemies, 1, rnn_hidden_dim]
        embedding_enemies = th.matmul(enemy_feats_t.unsqueeze(1), fc1_w_enemy).view(
            bs * self.n_agents, self.n_heads, self.n_enemies, self.rnn_hidden_dim
        )  # [bs * n_agents, head, n_enemies, rnn_hidden_dim]
        # Q * K^T
        key_enemies = self.WK_enemy(enemy_feats_t).reshape(bs * self.n_agents, self.n_enemies, self.rnn_hidden_dim)
        key_enemies = key_enemies.transpose(-1, -2)
        QK_enemy = th.matmul(query_own, key_enemies).unsqueeze(dim=1).expand(bs*self.n_agents, self.n_heads, 1, self.n_enemies) # (bs*n_agent, n_heads, 1, n_enemies)
        # V
        value_enemies = embedding_enemies
        embedding_enemies = th.matmul(self.attention_softmax(QK_enemy / np.sqrt(self.rnn_hidden_dim)), value_enemies)
        ...
        # [bs * n_agents * n_allies, 1, ally_fea_dim] * [bs * n_agents * n_allies, ally_fea_dim, rnn_hidden_dim] = [bs * n_agents * n_allies, 1, rnn_hidden_dim]
        embedding_allies = th.matmul(ally_feats_t.unsqueeze(1), fc1_w_ally).view(
            bs * self.n_agents, self.n_heads, self.n_allies, self.rnn_hidden_dim
        )  # [bs * n_agents, n_allies, rnn_hidden_dim * head]
        # Q * K^T
        key_allies = self.WK_ally(ally_feats_t).reshape(bs * self.n_agents, self.n_allies, self.rnn_hidden_dim)
        key_allies = key_allies.transpose(-1, -2)
        QK_ally = th.matmul(query_own, key_allies).unsqueeze(dim=1).expand(bs*self.n_agents, self.n_heads, 1, self.n_allies) # (bs*n_agent, n_heads, 1, n_allies)
        # V
        value_allies = embedding_allies
        embedding_allies = th.matmul(self.attention_softmax(QK_ally / np.sqrt(self.rnn_hidden_dim)), value_allies)
        # Final embedding
        embedding = embedding_own + self.unify_input_heads(embedding_enemies.squeeze(dim=2) + embedding_allies.squeeze(dim=2))
