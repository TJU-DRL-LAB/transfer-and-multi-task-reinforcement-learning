...
class Hypernet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, activation_func, is_out_acti):
        super(Hypernet, self).__init__()
        self.is_out_acti = is_out_acti
        if is_out_acti:
            self.net = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                get_activation_func(activation_func, hidden_dim),
                nn.Linear(hidden_dim, output_dim),
                get_activation_func('tanh', hidden_dim),
            )
            self.weight = Parameter(th.Tensor(output_dim).fill_(1.))
        else:
            self.net = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                get_activation_func(activation_func, hidden_dim),
...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.hyper_input_w_enemy = Hypernet(
            ...
            output_dim=self.enemy_feats_dim * self.rnn_hidden_dim, activation_func=args.api_hyper_activation,
            is_out_acti=args.api_hyper_out_acti
        ...
        self.hyper_input_w_ally = Hypernet(
            ...
            output_dim=self.ally_feats_dim * self.rnn_hidden_dim, activation_func=args.api_hyper_activation,
            is_out_acti=args.api_hyper_out_acti
        ...
        self.hyper_output_w_attack_action = Hypernet(
            input_dim=self.enemy_feats_dim, hidden_dim=args.api_hyper_dim,
            output_dim=self.rnn_hidden_dim, activation_func=args.api_hyper_activation,
            is_out_acti=args.api_hyper_out_acti
        )  # output shape: rnn_hidden_dim * 1
        self.hyper_output_b_attack_action = Hypernet(
            input_dim=self.enemy_feats_dim, hidden_dim=args.api_hyper_dim,
            output_dim=1, activation_func=args.api_hyper_activation,
            is_out_acti=args.api_hyper_out_acti
        )  # output shape: 1
    ...
    def forward(self, inputs, hidden_state):
        ...
        q_attack = (th.matmul(hh.unsqueeze(1), output_w_attack).squeeze(1) + output_b_attack).view(bs, self.n_agents,
