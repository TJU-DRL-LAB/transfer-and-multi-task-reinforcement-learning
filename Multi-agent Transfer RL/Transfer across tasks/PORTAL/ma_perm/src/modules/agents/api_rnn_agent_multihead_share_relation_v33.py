...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        # enemy_relation_network
        self.relation_enemy = nn.Sequential(
            nn.Linear(self.own_feats_dim+self.enemy_feats_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, self.n_heads),
            nn.Tanh()
        )
        # ally_relation_network
        self.relation_ally = nn.Sequential(
            nn.Linear(self.own_feats_dim+self.ally_feats_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, self.n_heads),
            nn.Tanh()
        )
    ...
    # get API feature
    def forward_for_feature(self, inputs, hidden_state):
        ...
        # (bs*n_agent, own_feats_dim) -> (bs*n_agent, n_enemies, own_feats_dim)
        ...
        relation_weight_enemy = self.relation_enemy(torch.cat([own_feats_t_expand_for_enemy, enemy_feats_t_expand], dim=2)).view(
            bs * self.n_agents, self.n_enemies, self.n_heads, 1
        )
        embedding_enemies = embedding_enemies * relation_weight_enemy
        ...
        own_feats_t_expand_for_ally = own_feats_t.unsqueeze(dim=1).expand(bs*self.n_agents, self.n_allies, self.own_feats_dim)
        ally_feats_t_expand = ally_feats_t.view(bs*self.n_agents, self.n_allies,self.ally_feats_dim)
        relation_weight_ally = self.relation_ally(torch.cat([own_feats_t_expand_for_ally, ally_feats_t_expand], dim=2)).view(
            bs * self.n_agents, self.n_allies, self.n_heads, 1
        )
        embedding_allies = embedding_allies * relation_weight_ally
    ...
    def forward(self, inputs, hidden_state):
        ...
        # peek_w_enemy = torch.mean(relation_weight_enemy, axis=2)
        ...
        #
        # peek_w_ally = torch.mean(relation_weight_ally, axis=2)
