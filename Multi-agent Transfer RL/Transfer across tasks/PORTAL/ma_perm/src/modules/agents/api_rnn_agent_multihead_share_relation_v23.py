...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.hyper_ally_feats_dim = self.ally_feats_dim
        self.hyper_enemy_feats_dim = self.enemy_feats_dim
        ...
        # HyperNet_dim
        # obs hypernet dim= enemy_feats_dim * rnn_hidden_dim # weight
        # action hypernet dim=rnn_hidden_dim * 1 + 1      # weight and bias
        # relation hypernet dim = (own_feats_dim+enemy_feats_dim) * 1
        enemy_hyper_dim = ((self.enemy_feats_dim + 1) * self.rnn_hidden_dim + 1 + self.own_feats_dim + self.enemy_feats_dim) * self.n_heads
        ...
        self.hyper_enemy = nn.Sequential(
            ...
            nn.Linear(args.api_hyper_dim, enemy_hyper_dim)
        ...
        if self.args.map_type == "MMM":
            ...
            self.hyper_ally = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Linear(args.api_hyper_dim, ((self.ally_feats_dim + 1) * self.rnn_hidden_dim + 1 + self.ally_feats_dim) * self.n_heads)
            ...
            # HyperNet_dim
            # obs hypernet dim= ally_feats_dim * rnn_hidden_dim # weight
            # relation hypernet dim = (own_feats_dim + enemy_feats_dim) * 1
            ally_hyper_dim = (self.ally_feats_dim * self.rnn_hidden_dim + self.own_feats_dim + self.ally_feats_dim) * self.n_heads
            self.hyper_ally = nn.Sequential(
                nn.Linear(self.ally_feats_dim, args.api_hyper_dim),
                nn.ReLU(inplace=True),
                nn.Linear(args.api_hyper_dim, ally_hyper_dim)
        ...
        # activation for relation weight
        self.relation_activation = torch.nn.Sigmoid()
    ...
    # get API feature
    """def forward_for_feature(self, inputs, near_n):
        # [bs, n_agents, mv_fea_dim], [bs * n_agents * n_enemies, enemy_fea_dim], [bs * n_agents * n_allies, ally_fea_dim], [bs, n_agents, own_fea_dim]
        bs, own_feats_t, enemy_feats_t1, ally_feats_t1, embedding_indices = inputs

        if near_n >= 0:
            # <0时，就是选取所有obs
            # feats with origin shape
            raw_enemy_feats_t = enemy_feats_t1.reshape(bs, self.n_agents, self.n_enemies, self.enemy_feats_dim)
            raw_ally_feats_t = ally_feats_t1.reshape(bs, self.n_agents, self.n_allies, self.enemy_feats_dim)
            enemy_feats_list = []
            ally_feats_list = []
            for bs_i in range(bs):
                for n_agent_i in range(self.n_agents):
                    # index=1是distance
                    enemy_feats_list.append(find_max_n(raw_enemy_feats_t[bs_i][n_agent_i], near_n, index=1))
                    ally_feats_list.append(find_max_n(raw_ally_feats_t[bs_i][n_agent_i], near_n-1, index=1))
            enemy_feats_t = torch.cat(enemy_feats_list)
            ally_feats_t = torch.cat(ally_feats_list)
            near_n_enemy = near_n
            near_n_ally = near_n - 1
        else:
            enemy_feats_t = enemy_feats_t1
            ally_feats_t = ally_feats_t1
            near_n_enemy = self.n_enemies
            near_n_ally = self.n_allies
        ...
        hyper_enemy_out = self.hyper_enemy(enemy_feats_t) # (bs*n_agent*n_enemy, ((enemy_feats_dim * rnn_hidden_dim + rnn_hidden_dim + 1)*n_head)
        fc1_w_enemy = hyper_enemy_out[:, :-(self.rnn_hidden_dim + 2) * self.n_heads].reshape(
            -1, self.enemy_feats_dim, self.rnn_hidden_dim * self.n_heads
        )  # [bs * n_agents * n_enemies, enemy_fea_dim, rnn_hidden_dim]
        # relation weight
        relation_w_enemy = hyper_enemy_out[:, -(self.rnn_hidden_dim + 2) * self.n_heads : -(self.rnn_hidden_dim + 1) * self.n_heads].view(
            bs * self.n_agents, near_n_enemy, self.n_heads, 1
        )
        relation_w_enemy = self.relation_activation(relation_w_enemy)
        ...
        embedding_enemies = th.matmul(enemy_feats_t.unsqueeze(1), fc1_w_enemy).view(
            bs * self.n_agents, near_n_enemy, self.n_heads, self.rnn_hidden_dim
        )  # [bs * n_agents, n_enemies, rnn_hidden_dim * head]
        embedding_enemies = embedding_enemies * relation_w_enemy
        ...
        if self.args.map_type == "MMM":
            fc1_w_ally = hyper_ally_out[:, :-(self.rnn_hidden_dim + 2) * self.n_heads].reshape(
                -1, self.ally_feats_dim, self.rnn_hidden_dim * self.n_heads
            )  # [bs * n_agents * n_allies, ally_fea_dim, rnn_hidden_dim * head]
            # relation weight
            relation_w_ally = hyper_ally_out[:, -(self.rnn_hidden_dim + 2) * self.n_heads : -(self.rnn_hidden_dim + 1) * self.n_heads].view(
                bs * self.n_agents, near_n_ally, self.n_heads, 1
            )
        else:
            fc1_w_ally = hyper_ally_out[:, : -1 * self.n_heads].view(-1, self.ally_feats_dim, self.rnn_hidden_dim * self.n_heads)
            relation_w_ally = hyper_ally_out[:, -1 * self.n_heads :].view(
                bs * self.n_agents, near_n_ally, self.n_heads, 1
            )
        relation_w_ally = self.relation_activation(relation_w_ally)
        ...
        embedding_allies = th.matmul(ally_feats_t.unsqueeze(1), fc1_w_ally).view(
            bs * self.n_agents, near_n_ally, self.n_heads, self.rnn_hidden_dim
        )  # [bs * n_agents, n_allies, rnn_hidden_dim * head]
        embedding_allies = embedding_allies * relation_w_ally
        embedding_allies = embedding_allies.sum(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]
        # embedding_allies = embedding_allies.mean(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]
        ...
        return embedding_copy, embedding_own, self.unify_input_heads(embedding_enemies), self.unify_input_heads(embedding_allies)"""

    def forward(self, inputs, hidden_state):
        # [bs, n_agents, mv_fea_dim], [bs * n_agents * n_enemies, enemy_fea_dim], [bs * n_agents * n_allies, ally_fea_dim], [bs, n_agents, own_fea_dim]
        bs, own_feats_t, enemy_feats_t, ally_feats_t, embedding_indices = inputs
        if isinstance(enemy_feats_t, list):
            enemy_feats_t = enemy_feats_t[0]
        # (1) Own feature
        embedding_own = self.fc1_own(own_feats_t)  # [bs * n_agents, rnn_hidden_dim]
        ...
        fc1_w_enemy = hyper_enemy_out[:, :-(self.rnn_hidden_dim + 1 + self.own_feats_dim + self.enemy_feats_dim) * self.n_heads].reshape(
            -1, self.enemy_feats_dim, self.rnn_hidden_dim * self.n_heads
        )  # [bs * n_agents * n_enemies, enemy_fea_dim, rnn_hidden_dim]
        # relation weight
        relation_w_enemy = hyper_enemy_out[:, -(self.rnn_hidden_dim + 1 + self.own_feats_dim + self.enemy_feats_dim) * self.n_heads: -(self.rnn_hidden_dim + 1) * self.n_heads].reshape(
            -1, self.own_feats_dim + self.enemy_feats_dim, 1 * self.n_heads
        )
        # 构造own_feats + enemy_feats
        own_feats_t_expand_for_enemy = own_feats_t.unsqueeze(dim=1).expand(bs*self.n_agents, self.n_enemies, self.own_feats_dim)
        enemy_feats_t_expand = enemy_feats_t.view(bs*self.n_agents, self.n_enemies,self.enemy_feats_dim)
        concat_own_enemy_feats = torch.cat([own_feats_t_expand_for_enemy, enemy_feats_t_expand], dim=2).reshape(bs*self.n_agents*self.n_enemies, self.own_feats_dim+self.enemy_feats_dim)
        relation_enemy = th.matmul(concat_own_enemy_feats.unsqueeze(1), relation_w_enemy).view(
            bs * self.n_agents, self.n_enemies, self.n_heads, 1
        )
        relation_enemy = self.relation_activation(relation_enemy)
        # [bs * n_agents * n_enemies, 1, enemy_fea_dim] * [bs * n_agents * n_enemies, enemy_fea_dim, rnn_hidden_dim] = [bs * n_agents * n_enemies, 1, rnn_hidden_dim]
        embedding_enemies = th.matmul(enemy_feats_t.unsqueeze(1), fc1_w_enemy).view(
            bs * self.n_agents, self.n_enemies, self.n_heads, self.rnn_hidden_dim
        )  # [bs * n_agents, n_enemies, rnn_hidden_dim * head]
        embedding_enemies = embedding_enemies * relation_enemy
        embedding_enemies = embedding_enemies.sum(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]
        # (4) Ally features
        hyper_ally_out = self.hyper_ally(ally_feats_t)
        if self.args.map_type == "MMM":
            ...
            fc1_w_ally = hyper_ally_out[:, : -self.ally_feats_dim * self.n_heads].view(-1, self.ally_feats_dim, self.rnn_hidden_dim * self.n_heads)
            relation_w_ally = hyper_ally_out[:, -self.ally_feats_dim * self.n_heads:].reshape(
                -1, self.ally_feats_dim, 1 * self.n_heads
            )
        relation_ally = th.matmul(ally_feats_t.unsqueeze(1), relation_w_ally).view(
            bs * self.n_agents, self.n_allies, self.n_heads, 1
        )
        relation_ally = self.relation_activation(relation_ally)

        # [bs * n_agents * n_allies, 1, ally_fea_dim] * [bs * n_agents * n_allies, ally_fea_dim, rnn_hidden_dim] = [bs * n_agents * n_allies, 1, rnn_hidden_dim]
        embedding_allies = th.matmul(ally_feats_t.unsqueeze(1), fc1_w_ally).view(
            bs * self.n_agents, self.n_allies, self.n_heads, self.rnn_hidden_dim
        )  # [bs * n_agents, n_allies, rnn_hidden_dim * head]
        embedding_allies = embedding_allies * relation_ally
        embedding_allies = embedding_allies.sum(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]
        # embedding_allies = embedding_allies.mean(dim=1, keepdim=False)  # [bs * n_agents, head, rnn_hidden_dim]

        # peek_w_enemy = torch.mean(relation_w_enemy, axis=2)
        # max_peek_w_enemy = torch.max(peek_w_enemy, axis=1).values
        # min_peek_w_enemy = torch.min(peek_w_enemy, axis=1).values
        # diff_w_enemy = max_peek_w_enemy- min_peek_w_enemy
        # max_differ_w_enemy = torch.max(diff_w_enemy, axis=0).values
        #
        # peek_w_ally = torch.mean(relation_w_ally, axis=2)
        # max_peek_w_ally = torch.max(peek_w_ally, axis=1).values
        # min_peek_w_ally = torch.min(peek_w_ally, axis=1).values
        # diff_w_ally = max_peek_w_ally - min_peek_w_ally
        # max_differ_w_ally = torch.max(diff_w_ally, axis=0).values
...
def find_max_n(input_tensor, n, index):
    new_tensor = []
    ...
    for _ in range(len(new_tensor), n):
        new_tensor.append(torch.zeros(input_tensor.shape[1]))

    new_tensor = torch.stack(new_tensor, dim=0)
    values = []
    for row in new_tensor:
        values.append(row[index])
    ascent_indexs = np.argsort(np.array(values))
    return new_tensor[ascent_indexs[:n],:]