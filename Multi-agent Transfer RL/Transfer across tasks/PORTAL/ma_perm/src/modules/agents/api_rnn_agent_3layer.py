...
class API_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.hyper_input_w_enemy = nn.Sequential(
            nn.Linear(self.enemy_feats_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.api_hyper_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, self.enemy_feats_dim * self.rnn_hidden_dim)
        )  # output shape: (enemy_feats_dim * self.rnn_hidden_dim)
        self.hyper_input_w_ally = nn.Sequential(
        ...
        # Multiple entities (use hyper net to process these features to ensure permutation invariant)
        self.hyper_output_w_attack_action = nn.Sequential(
            nn.Linear(self.enemy_feats_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.api_hyper_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
        ...
        self.hyper_output_b_attack_action = nn.Sequential(
            nn.Linear(self.enemy_feats_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.api_hyper_dim, args.api_hyper_dim),
            nn.ReLU(inplace=True),
            nn.Linear(args.api_hyper_dim, 1)
        )  # output shape: 1
