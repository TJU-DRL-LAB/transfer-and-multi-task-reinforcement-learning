...
class API_RNNAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state):
        ...
        relation_weight_enemy = self.relation_enemy(enemy_feats_t).view(
            bs * self.n_agents, self.n_enemies, self.n_heads, 1
        )
        # Softmax
        relation_weight_enemy = self.weight_softmax(relation_weight_enemy / self.temperature)
        ...
        relation_weight_ally = self.relation_ally(ally_feats_t).view(
            bs * self.n_agents, self.n_allies, self.n_heads, 1
        )
        # Softmax
        relation_weight_ally = self.weight_softmax(relation_weight_ally / self.temperature)
