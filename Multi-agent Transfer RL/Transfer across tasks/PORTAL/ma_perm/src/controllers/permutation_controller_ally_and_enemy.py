...
class TransformationAgent(nn.Module):
    ...
    def forward(self, ally_feats, enemy_feats, sample_action, test_mode, t_env):
        ...
        for position_idx in range(n_allies):  # over n_position
            ...
            _ally_prob = self._generate_action_prob(logit, _invalid_ally_mask, test_mode)  # [bs, n_agents, n_allies]
        ...
        enemy_assignments = []
        for position_idx in range(n_enemies):  # over n_position
        ...
        # Inverse of assignment matrix
        inverse_enemy_assignments = enemy_assignments.transpose(-2, -1)  # [bs, n_agents, n_position, n_enemies]
        return ally_assignments, enemy_assignments, inverse_enemy_assignments
...
class EffMACOBS(BasicMAC):
    def __init__(self, scheme, groups, args):
        super(EffMACOBS, self).__init__(scheme, groups, args)
    ...
    def forward(self, ep_batch, t, sample_action, test_mode, use_stored, t_env):
        ...
        input_finish_time = time.time()
        # if use_stored:
        #     print("Build input costs {} seconds.".format(input_finish_time - start_time))
        ...
        reversed_attack_qs = th.matmul(
            self.inverse_enemy_assignments.transpose(-2, -1), attack_qs
        ).squeeze(-1)  # [bs, n_agent, n_enemies]
        # if use_stored:
        #     print("Ouput Q costs {} seconds.".format(time.time() - input_finish_time))
    ...
    def _build_inputs(self, batch, t, sample_action, test_mode, use_stored, t_env):
        ...
        if t % self.args.permute_condition_value == 0:
            if use_stored:  # Training
                ...
                ally_assignments, enemy_assignments, inverse_enemy_assignments = self.input_transformation_agent.forward(
                    _ally_feats_t,
                    _enemy_feats_t,
            ...
            if self.args.detach_inverse_assignment:
                inverse_enemy_assignments = inverse_enemy_assignments.detach()

            self.transposed_ally_assignments = th.transpose(ally_assignments, -1, -2)  # [bs, n_agents, n_pos, n_allies]
            self.transposed_enemy_assignments = th.transpose(enemy_assignments, -1, -2)  # [bs,n_agents,n_pos,n_enemies]
            self.inverse_enemy_assignments = inverse_enemy_assignments  # [bs, n_agents, n_position, n_enemies]
        # %%%%%%%%%%%%%%%%%%%%%%%%%%%% compute assignment matrix %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def prepare_permuted_q_for_training(self, batch, use_stored, is_target=False):
        ...
        if use_stored:  # Training
            ...
            inverse_enemy_assignments = enemy_assignments.transpose(-2, -1)  # [bs, traj,n_agents,n_position,n_enemies]
        else:
        ...
        inverse_enemy_assignments = inverse_enemy_assignments.view(
        ...
        for t in range(traj):
            ...
            inversed_attack_qs = th.matmul(
                inverse_enemy_assignments[:, t].transpose(-2, -1), attack_qs
            ).squeeze(-1)  # [bs, n_agent, n_enemies]
