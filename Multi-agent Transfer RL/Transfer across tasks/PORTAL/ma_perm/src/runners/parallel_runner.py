...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            if save_probs:
                ...
                actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,
                                                  test_mode=test_mode)
            #  是否有reconstruction loss
            # if type(actions) == tuple:
            #     rec_obs = actions[1]
            #     actions = actions[0]
            #     cpu_rec_obs = rec_obs.to("cpu").numpy()
            #     rec_obs_add = {
            #         "rec_obs": np.expand_dims(cpu_rec_obs, axis=1),
            #     }
            #     self.batch.update(rec_obs_add, bs=envs_not_terminated, ts=self.t, mark_filled=False)
            ...
            cof = 1
            if hasattr(self.mac.args,'cof'):
                cof = cof * self.mac.args.cof
            # Receive data back for each unterminated env
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    data = parent_conn.recv()
                    # Remaining data for this current timestep
                    # 为了transfer添加系数, 注意下面这个,不能少
                    post_transition_data["reward"].append((data["reward"]*cof,))
        ...
        # 修改batch_size==1的情况，同样记录info，不太优雅
        if test_mode and (len(self.test_returns) == n_test_runs) or self.batch_size == 1:
            # 判断是否收敛
            if self.args.run_type == 'train_converge':
                if np.mean(self.test_returns) > self.max_mean:
                    self.max_mean = np.mean(self.test_returns)
                    self.max_mean_step = self.t_env
                    self.cnt = 0
                else:
                    self.cnt += 1
                if self.cnt >= self.MAX_CNT or self.max_mean == 20:
                    self.converge = True
                    print(f'converged at step {self.max_mean_step}')
                    return
