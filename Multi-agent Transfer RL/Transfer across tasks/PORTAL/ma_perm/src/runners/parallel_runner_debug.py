...
class ParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        # %%%%%%%%% Debug time %%%%%%%%%%%
        self.__time__interact_with_env = 0
        self.__time__copy_model_to_cpu = 0
        self.__time__model_inference = 0
        self.__time__generate_batch_traj = 0

    def get_time(self):
        return self.__time__interact_with_env, self.__time__copy_model_to_cpu, self.__time__model_inference, self.__time__generate_batch_traj
    ...
    def reset(self):
        ...
        __t_2 = time.time()
        if str(self.mac.get_device()) != "cpu":
            self.mac.cpu()  # TODO: copy model to cpu
        __t_3 = time.time()
        ...
        __t_5 = time.time()
        self.__time__generate_batch_traj += (__t_2 - __t_1)
        self.__time__copy_model_to_cpu += (__t_3 - __t_2)
        self.__time__interact_with_env += (__t_4 - __t_3)
        self.__time__generate_batch_traj += (__t_5 - __t_4)
    ...
    def run(self, test_mode=False):
        ...
        __t_2 = time.time()
        self.__time__model_inference += (__t_2 - __t_1)
        ...
        while True:
            ...
            if save_probs:
                ...
                actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,
                                                  test_mode=test_mode)
            __t_4 = time.time()
            self.__time__model_inference += (__t_4 - __t_3)

            cpu_actions = actions.to("cpu").numpy()

            # Update the actions taken
            actions_chosen = {
                "actions": np.expand_dims(cpu_actions, axis=1),  # actions.unsqueeze(1).to("cpu"),
            }
            if save_probs:
            ...
            __t_6 = time.time()
            self.__time__interact_with_env += (__t_6 - __t_5)
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    pre_transition_data["obs"].append(data["obs"])
            __t_7 = time.time()
            self.__time__interact_with_env += (__t_7 - __t_6)
            ...
            __t_8 = time.time()
            self.__time__generate_batch_traj += (__t_8 - __t_7)
