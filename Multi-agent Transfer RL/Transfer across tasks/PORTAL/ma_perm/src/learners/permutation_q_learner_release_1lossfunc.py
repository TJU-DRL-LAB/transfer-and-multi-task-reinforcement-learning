...
def calculate_target_q(target_mac, batch, enable_parallel_computing=False, thread_num=4):
    ...
    with th.no_grad():
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs = target_mac.forward(batch, t=t, is_sample_action=False, is_target_net=True, t_env=-1)
...
class PermutationQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # Q-net params
        q_net_params = list(self.mac.q_net_parameters()) + list(self.mixer.parameters())
        self.q_net_optimizer = Adam(params=q_net_params, lr=self.args.lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs = self.mac.forward(batch, t=t, is_sample_action=False, is_target_net=False, t_env=t_env)
        ...
        # %%%%%%%%%%%%%% Auxiliary-loss %%%%%%%%%%%%%
        greedy_q_values = mac_out[:, :-1].max(dim=-1, keepdim=False)[0]  # [bs, T-1, n_agents]
        # greedy_q_tot = greedy_q_values.sum(dim=-1, keepdim=True)  # [bs, T-1, 1]
        greedy_q_tot = self.mixer(greedy_q_values, batch["state"][:, :-1])
        assignment_loss = - self.args.auxiliary_loss_coef * (greedy_q_tot * mask).sum() / mask_elems

        # Optimise
        # (1) Clear all gradients;
        self.optimiser.zero_grad()

        # (2) Back-propagate gradients of auxiliary loss;
        assignment_loss.backward(retain_graph=True)

        # (3) Clear the gradients of auxiliary loss for the Q-net parameters (make sure the assignment_loss is only used to update the assignment network);
        self.q_net_optimizer.zero_grad()

        # (4) Back-propagate gradients of TD loss;
        td_loss.backward()

        grad_norm = th.nn.utils.clip_grad_norm_(self.params, self.args.grad_norm_clip)
        # (5) Apply the gradients update.
        self.optimiser.step()

        self.train_t += 1
        self.avg_time += (time.time() - start_time - self.avg_time) / self.train_t
        print("Avg cost {} seconds, td_loss={}, assignment_loss={}".format(self.avg_time, td_loss.item(), assignment_loss.item()))
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("loss_assignment", assignment_loss.item(), t_env)
