...
class NQLearner:
    ...
    def train_each_split(self, batch, split_num=1):
        ...
        mask_elems = mask.sum()
        loss = masked_td_error.sum() / mask_elems / split_num
        loss.backward()

        # For log
        with th.no_grad():
            td_error_abs = (masked_td_error.abs().sum() / mask_elems).item() / split_num
            q_taken_mean = (chosen_action_qvals * mask).sum() / (mask_elems * self.args.n_agents).item() / split_num
            target_mean = (targets * mask).sum() / (mask_elems * self.args.n_agents).item() / split_num
        return loss, td_error_abs, q_taken_mean, target_mean
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        split_num = 4 if self.args.n_agents > 20 else 2
        split_data = batch.split(split_num)
        loss, td_error_abs, q_taken_mean, target_mean = 0, 0, 0, 0
        for sub_batch in split_data:
            result = self.train_each_split(sub_batch, split_num)
            loss += result[0]
            td_error_abs += result[1]
            q_taken_mean += result[2]
            target_mean += result[3]
