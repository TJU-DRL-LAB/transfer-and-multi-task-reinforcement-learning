...
def calculate_target(target_mac, batch, using_stored_assignment, mixer):
    """
    ...
    with th.no_grad():
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs = target_mac.forward(
                ...
                sample_action=False,
                test_mode=False,
                use_stored=using_stored_assignment,
                # use_stored=False,
                t_env=-1
            )
        ...
        return target_mac_out, target_q_states
...
def calculate_permuted_q_for_target(target_mac, batch, using_stored_assignment, mixer):
    ...
    with th.no_grad():
        ...
        return target_mac.prepare_permuted_q_for_training(batch, use_stored=using_stored_assignment,
                                                          is_target=True), target_q_states
...
class PermutationQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        target_mac_out = self.pool.apply_async(
            calculate_target,
            (self.target_mac, batch, self.args.update_Q_using_stored_assignment, self.args.mixer)
        )
        # TODO: %%%%%%%%%%%%%%%% 并行：开另一个进程计算 target %%%%%%%%%%%%%%%%%%%%%%%%%%%%

        # mac_out = self.mac.prepare_permuted_q_for_training(batch, use_stored=self.args.update_Q_using_stored_assignment)
        ...
        for t in range(batch.max_seq_length):
            agent_outs = self.mac.forward(
                ...
                sample_action=False,
                test_mode=False,
                use_stored=self.args.update_Q_using_stored_assignment,
                t_env=-1
            )
        ...
        if train_auxiliary_flag:
            ...
            # _mac_out = self.mac.prepare_permuted_q_for_training(batch, use_stored=False)[:, :-1]
            _mac_out = []
            self.mac.init_hidden(batch.batch_size)
            for t in range(batch.max_seq_length):
                agent_outs = self.mac.forward(
                    ...
                    use_stored=False,  # on-policy assignment
                    t_env=-1
                )
                _mac_out.append(agent_outs)
            _mac_out = th.stack(_mac_out, dim=1)[:, :-1]  # Concat over time

            _mac_out[avail_actions[:, :-1] == 0] = -9999999  # mask out invalid actions
            greedy_q_values = _mac_out.max(dim=-1, keepdim=False)[0]  # [bs, T-1, n_agents]
