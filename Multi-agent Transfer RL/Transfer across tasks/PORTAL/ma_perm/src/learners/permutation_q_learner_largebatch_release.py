...
def calculate_target_q(target_mac, batch, enable_parallel_computing=False, thread_num=4):
    ...
    with th.no_grad():
        ...
        target_mac_out = target_mac.prepare_permuted_q_for_training(batch, is_target_net=True)
...
class PermutationQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.params = list(mac.q_net_parameters())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = self.mac.prepare_permuted_q_for_training(batch, is_target_net=False)
        ...
        # (2) Back-propagate gradients of TD loss;
        td_loss.backward()
        grad_norm = th.nn.utils.clip_grad_norm_(self.params, self.args.grad_norm_clip)
        # (3) Apply the gradients update.
        self.optimiser.step()

        # %%%%%%%%%%%%%% Auxiliary-loss %%%%%%%%%%%%%
        _mac_out = self.mac.prepare_permuted_q_for_training(batch, is_target_net=False)
        _mac_out[avail_actions == 0] = -9999999  # mask out invalid actions

        greedy_q_values = _mac_out[:, :-1].max(dim=-1, keepdim=False)[0]  # [bs, T-1, n_agents]
        ...
        print("[V3] Avg cost {} seconds, td_loss={}, assignment_loss={}".format(self.avg_time, td_loss.item(),
                                                                                assignment_loss.item()))
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("grad_norm_assign", transform_grad_norm, t_env)
