...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
            ...
            debug=False,
            use_modified_delta_health=False,
    ):
        """
        ...
        """
        # print("to check if this file is used")
        # Map arguments
        ...
        self.use_modified_delta_health = use_modified_delta_health
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                ...
                if e_unit.health == 0:
                    ...
                    # TODO: max(delta_health, 0): we should only consider the health drop (including the regenerated one) caused by ally agents !!!
                    delta_enemy_health += max(prev_health - e_unit.health, 0)
                    # TODO: max(delta_shield, 0): we should only consider the shield drop (including the regenerated one) caused by ally agents !!!

                    delta_enemy_shield += max(prev_shield - e_unit.shield, 0)
                    delta_enemy += (prev_health + prev_shield - e_unit.health - e_unit.shield)

        if self.reward_only_positive:
            if self.use_modified_delta_health:
                reward = delta_enemy_health + delta_enemy_shield + delta_deaths  # our modified one
            else:
                reward = abs(delta_enemy + delta_deaths)  # shield regeneration (Original setting, the 'abs' is improper)
        else:
            if self.use_modified_delta_health:
                # our modified one, only consider the regenerated shield and blood drops of the enemies.
                reward = delta_enemy_health + delta_enemy_shield + delta_deaths - delta_ally
            else:
                ...
                # in summary: the recovered blood or shield has no effect on the overall Return.u
    ...
    # 返回agent种类和数目
    # 如果返回None,说明该环境单一兵种
    def get_unit_type_and_num(self):
        type_of_obs = {}
        if self.unit_type_bits > 0:
            type_enemy = {}
            for e_id, e_unit in self.enemies.items():
                type_id = self.get_unit_type_id(e_unit, False)
                type_enemy.setdefault(type_id, 0)
                type_enemy[type_id] += 1
            type_of_obs['enemy'] = type_enemy
            type_ally = {}
            for i in range(self.n_agents):
                unit = self.get_unit_by_id(i)
                type_id = self.get_unit_type_id(unit, True)
                type_ally.setdefault(type_id, 0)
                type_ally[type_id] += 1
            type_of_obs['ally'] = type_ally
            return type_of_obs
        return None
    ...
    # wjz add
    def init_units_sorted(self):
        """initialise the units in order"""
        while True:
            ...
            if all_agents_created and all_enemies_created:  # all good
                # print("all_agents_created")
                # print(f"enemies: {[(k,v.unit_type) for k,v in self.enemies.items()]}")
                # print(f"agents: {[(k,v.unit_type) for k,v in self.agents.items()]}")
    ...
    # 进一步划分兵种
    # 友军先不划分
    def get_obs_component_unit_type(self):
        move_feats_dim = self.get_obs_move_feats_size()
        enemy_feats_dim = self.get_obs_enemy_feats_size()
        ally_feats_dim = self.get_obs_ally_feats_size()
        own_feats_dim = self.get_obs_own_feats_size()
        type_info = self.get_unit_type_and_num()
        obs_component = [move_feats_dim, enemy_feats_dim, ally_feats_dim, own_feats_dim]
        if type_info:
            # {'enemy': {0: 3, 1: 2}, 'ally': {0: 2, 1: 3}}
            feats_dim_divide = []
            for team, type_num in type_info.items():
                for type, num in type_num.items():
                    if team == 'enemy':
                        feats_dim_divide.append((num, enemy_feats_dim[1]))
                    # else:
                    #     feats_dim_divide.append((num, ally_feats_dim[1]))

            # 不考虑友军划分
            obs_component_divide = [move_feats_dim] + feats_dim_divide + [ally_feats_dim, own_feats_dim]
            return obs_component_divide
        return obs_component
